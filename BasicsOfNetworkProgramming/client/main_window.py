...
class ClientMainWindow(QMainWindow):
    ...
    def set_disabled_input(self):
        # Поля ввода не активны по умолчанию
    ...
    def history_list_update(self):
        # Заполнение окна историей переписки с текущим собеседником
    ...
    def add_contact_window(self):
        # Создание диалога добавления контакта
    ...
    def add_contact_action(self, item):
        # Обработчик нажатия "Добавить"
        new_contact = item.selector.currentText()
        self.add_contact(new_contact)
        item.close()

    def add_contact(self, new_contact):
        # Добавление контакта в серверную и клиентскую БД. После обновления баз данных обновляет и содержимое окна.
        try:
    ...
    def delete_contact(self, item):
        # Удаление контакта из серверной и клиентской БД. После обновления баз данных обновляет и содержимое окна.
    ...
    def send_message(self):
        # Отправка сообщения текущему сообщению
    ...
    def message(self, message):
        # Слот обрабатывает поступаемые сообщения. Запрос пользователя если сообщение не от текущего собеседника
        ...
        if sender == self.current_chat:
            ...
            if self.database.check_contact(sender):
                ...
                if self.messages.question(
                    ...
                    self.current_chat = sender
                    # Сохранить сообщение, иначе оно будет потеряно, т.к. на момент предыдущего вызова контакта не было.
                    self.database.save_message(
    ...
    @pyqtSlot()
    def sig_205(self):
        # Обновление БД по команде сервера
