...
class ClientDatabase:
    class KnownUsers:
        # Отображение для таблицы всех пользователей.
        def __init__(self, user):
            self.id = None
            self.username = user

    class MessageStat:
        # Отображение для таблицы статистики переданных сообщений
    ...
    class Contacts:
        # Отображение для таблицы контактов.
    ...
    def contacts_clear(self):
        # Очищаем таблицу с контактами
        self.session.query(self.Contacts).delete()
        self.session.commit()

    def del_contact(self, contact):
        # Удаляем определенный контакт
        self.session.query(self.Contacts).filter_by(name=contact).delete()
        self.session.commit()

    def add_users(self, users_list):
        # Заполняем таблицу известных пользователей
    ...
    def get_users(self):
        # Возвращаем список всех известных пользователей
        return [user[0] for user in self.session.query(self.KnownUsers.username).all()]

    def check_user(self, user):
        # Проверяем существует ли пользователей
    ...
    def get_history(self, contact):
        # Возвращаем историю сообщений с определенным пользователем
        query = self.session.query(self.MessageStat).filter_by(contact=contact)
        return [(history_row.contact, history_row.direction, history_row.message, history_row.date) for history_row in
                query.all()]
